...
import { buildCollectionSearchParams, getCollectionViewLabel, parseCollectionQueryState } from "@/lib/query-state";
...
describe("query-state helpers", () => {
  ...
  it("parses collection state with beauty sort and compact view", () => {
    expect(parseCollectionQueryState({ sortBy: "beauty", view: "compact", page: "3" })).toMatchObject({
      sortBy: "beauty",
      view: "compact",
      page: 3
    });
  ...
  it("parses collection state with array values gracefully", () => {
    expect(parseCollectionQueryState({ address: ["a", "b"], query: ["1"], page: ["2"] })).toMatchObject({
      address: undefined,
      query: undefined,
      page: 1
    });
  ...
  it("handles parsePositiveNumber with valid zero", () => {
    expect(parseCollectionQueryState({ query: "0" })).toMatchObject({ query: 0 });
  });

  it("treats empty query string as undefined, not zero", () => {
    expect(parseCollectionQueryState({ query: "" })).toMatchObject({ query: undefined });
  });
