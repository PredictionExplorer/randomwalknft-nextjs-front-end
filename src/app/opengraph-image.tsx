...
export default async function OpenGraphImage() {
  ...
  const hours = vault
    ? Math.max(0, Math.floor((vault.secondsUntilWithdrawal % 86_400) / 3_600))
    : null;
    ...
    (
      ...
      >
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ fontSize: 30, color: "#c676d7", letterSpacing: "0.35em" }}>
          ...
          <div style={{ maxWidth: 900, marginTop: 20, fontSize: 30, color: "#e8c9f2", lineHeight: 1.4 }}>
            Every mint draws a unique artwork from an on-chain seed — and the last minter standing
            takes half the vault.
          </div>
        </div>
          ...
          <div style={{ display: "flex", gap: 56 }}>
            {stats.map((item) => (
              <div key={item.label} style={{ display: "flex", flexDirection: "column" }}>
                <div style={{ fontSize: 22, color: "#9b8ba3", letterSpacing: "0.25em" }}>
                  {item.label}
                </div>
                <div style={{ fontSize: 46, fontWeight: 700, color: "#f4bfff", marginTop: 8 }}>
                  {item.value}
                </div>
              </div>
            ))}
