export const generatorFiles = {
  ...
  sourceFileName: "randomWalkGen.py",
  readme: `You need to install opencv. On an Ubuntu machine you would need to run the following command:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 randomWalkGen.py <NFT number>")
        sys.exit(1)

    token_id = int(sys.argv[1])
    seed = get_seed(token_id)
    file_name = str(token_id).zfill(6)
